...
# -*- coding: utf-8 -*-
"""
wx-html —— Markdown → 微信公众号 HTML 排版。

**来源**：content-pipeline 的 `format` 子命令（`content_pipeline.pipeline.format`），
几乎原样移植，仅将子命令名改为 `wx-html`。
提供可编程接口 `render_markdown_file()` / `render_markdown_text()`，
其余（THEMES / build_css / 转换 / sanitize / build_html）保持一致。
"""
...
def run(args):
    out = render_markdown_file(
        args.input, theme=args.theme, font_size=args.font_size,
        output=args.output, inline=args.inline)
    print(f'✓ {out}')
    print(f'  theme={args.theme}  font-size={args.font_size} ({FONT_SIZES[args.font_size]})')
    if not HAS_MARKDOWN:
        print('  ⚠ 未安装 markdown，使用内置转换器（pip install markdown 效果更好）')


def reg_subparser(subparsers):
    parser = subparsers.add_parser(
        "wx-html", help="Markdown → 微信公众号 HTML 排版",
        formatter_class=RawTextHelpFormatter,
        epilog='示例：BookerMarkdownTool wx-html article.md --theme 01fish --font-size medium')
    parser.add_argument("input", help="Markdown 文件路径")
    parser.add_argument("--theme", choices=list(THEMES.keys()), default='01fish',
                        help='配色主题 (default: 01fish)')
    ...
    parser.add_argument("--inline", action="store_true",
                        help='内联 CSS 到元素 style（用于微信 API 推送，需 premailer）')
